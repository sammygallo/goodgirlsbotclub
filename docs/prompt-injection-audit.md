...
- **Stage C — post-history**, one system message per section (`:1683-1689`): `char_phi · user_phi · wi_after_an · ext_after_an` (`POST_HISTORY_SECTIONS`, `generationStore.ts:166-171`). Stage membership is not reorderable — dragging `char_phi` to the top only moves it among the post-history four.
- **Delivery past the builder (#509, fixed by E9-S10):** the backend relay forwards this array untouched to the eleven `openai`-family providers (`openrouter` included) and to the `custom` source. For the `anthropic` and `google` families it hoists only the LEADING run of system messages into the provider's out-of-band system field; every later system message — the character's note at its default system role, WI `at_depth`, every Stage-C section — stays at its index, delivered as a user turn wrapped as a system note (`ggbc-backend/app/providers/system_placement.py`). Before E9-S10 both translators hoisted every system-role message by role, so on those two families nothing above landed at depth and Stage C was not last.
