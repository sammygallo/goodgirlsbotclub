...
- **Before the review stage and after every round: NO MODIFIED TRACKED FILES, in every repo the story touches.** Run `git status --porcelain --untracked-files=all` plus `git status --porcelain --ignored=matching -- .claude/worktrees` and paste both into the run record. The standard is asymmetric on purpose:
  - **Tracked paths: zero modifications, every time — including the pre-review run.** Not "unchanged since a baseline": a baseline that already lists a modified file makes a reviewer's in-place mutation of that same file invisible, since porcelain reports paths and status codes, never content. Any modified tracked path stops the pipeline until explained and reverted — that is the E1-S1 mutation-left-in-the-shared-checkout failure this gate exists to catch, and a dirty starting tree is itself the contested-checkout condition that must be resolved before review, not tolerated.
  - **Untracked paths: enumerate the expected ones before review; any NEW untracked path afterwards stops the pipeline.** A long-lived untracked local file is not a violation, but it must be named up front rather than waved through. Never silence one with `.gitignore` — an ignored file drops out of ripgrep's default traversal, silently breaking the INTAKE/absorption carrier sweeps that must reach "any handoff doc", and `.claude/.gitignore` already hides `worktrees/` from plain porcelain, which is why the `--ignored=matching` probe above is not optional.
  - `git worktree list` remains the secondary sweep for stray registrations; it cannot see an in-place mutation, so it never substitutes for the tracked-clean check. This is the check the prose rule lacked when it failed twice (E1-S1's in-place mutation, which `git worktree list` cannot see; E2-S2's stray `skeptic-wt` worktree). `git worktree list` is the secondary sweep for stray registrations; a worktree under `.claude/worktrees/` that no live session owns is a finding, not an exemption.
