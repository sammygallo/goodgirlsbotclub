...
| Group chats | **never** (`serverRetrieval.ts:32-34`, by design) | yes (identity = `characterAvatars[0]`, `chatStore.ts:901-903`) |
...
**F6 — `reason: "no_key"` never surfaced.** The backend distinguishes "no embedding key configured" from "no matches", but the client discards the field (`chatStore.ts:925` region), so a user with recall enabled and no key sees silence.
