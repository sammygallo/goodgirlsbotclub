...
- **Delivery past the builder (#509, fixed by E9-S10):** the backend relay forwards this array untouched to the eleven `openai`-family providers (`openrouter` included). For the `anthropic` and `google` families it hoists only the LEADING run of system messages into the provider's out-of-band system field; every later system message — the character's note at its default system role, WI `at_depth`, every Stage-C section — stays at its index, delivered as a user turn wrapped as a system note (`ggbc-backend/app/providers/system_placement.py`). Before E9-S10 both translators hoisted every system-role message by role, so on those two families nothing above landed at depth and Stage C was not last.
