...
| **E2-S5** WI telemetry coverage | Transparency | Pre-cutover `wi_fired` maps remapped or flagged partial-coverage, and `wiReplay` able to say "not observable" rather than "never fired" — hard-blocks E5-S1. **Re-scoped 2026-08-30:** the original id-crosswalk task is a no-op since the 2026-08-23 cutover (§1) |
...
**Re-scoped 2026-08-30 at intake; the original "id crosswalk" premise is dead — see §1.** The card was written from a `captureWiFired` comment dated 2026-08-06 asserting backend UUIDs are permanently disjoint from local `wi_`/`wibook_` ids. The 2026-08-23 memory-consolidation cutover made native `/lorebooks` the system of record and collapsed the two schemes, so a server-path firing is **already** recorded under the local entry's id and the crosswalk task is a no-op. That comment is corrected in-tree so the dead premise cannot be re-derived from code.

What remains is telemetry *coverage*, in two independent gaps. **(1) Pre-cutover telemetry is orphaned.** `wi_fired` maps persisted in chat headers before 2026-08-23 are keyed on `wi_`/`wibook_` ids and nothing remaps them: `worldInfoStore` exports `remapLegacyEntryId`/`resolveLegacyEntryId` for precisely this and `chatStore` calls neither, so historical firings silently read as never-fired — the exact false signal E5-S1 must not build an "unused entry" finding on. **(2) `wiReplay` cannot say "not observable".** `neverFired` is active entries with no `fired[key]` (`wiReplay.ts:169`), which collapses "we watched and it never fired" into the same value as "we hold no telemetry for this chat at all"; `approximate` is a single boolean over the whole map and cannot carry the distinction either.
