...
**Size bands (output tokens): S** ≤150k · **M** 150–500k · **L** 0.5–1.5M · **XL** >1.5M. **Recalibrated 2026-08-28 (Sammy-directed, on six 10.2 runs of actuals): the letters price the INITIAL build only.** Fix-round rework is verification-driven spend — it exists because a review round found something — and is budgeted with the review loop below, never by padding the build letter. E2-S2 is the worked example: its initial build legs ran on-band while five finding-driven fix rounds (~0.3–0.85M each) carried the build-side total to ~6.9M.

**Verification is priced in ROUNDS × a unit cost, not in letters** (charter §4.2: checkable claims × rounds-to-convergence). Unit costs, stable across six runs: a **full trigger-tier round** (4 lenses + 2 skeptics per finding + mutation passes) costs **~1.5–3M**; a **scoped confirmation round** (reduced lenses over the delta — allowed only after a zero-confirmed-findings round, per `run-story` §5 as amended in #485) costs **~0.2–0.5M**; a **standard-tier `/code-review` pass** costs **≤0.3M**. Expected rounds by risk class:

| Risk class | Expected rounds | Verify budget | Evidence |
|---|---|---|---|
| Standard (S/M build, no §6.1 trigger) | 1 standard pass | ≤0.3M | — |
| Trigger-tier, contained seam | 1 full + 1 confirmation | ~2–3.5M | E1-S1: ~1.3M (1 round sufficed) |
| Safety gate / backend contract | 2–3 full + confirmation | ~3.5–9M | E4-S0: ~5.0M |
| Prompt assembly / new mechanism over a frozen layer | 4–6 full + confirmation | ~7–15M | E9-S6: 9.18M (4 rounds) · E2-S2: ~11M in-session (5 full + 1 scoped) |

The loop runs to convergence — the final round confirms zero — and a verification budget is never traded away to keep a story in band: the story slips, the review does not shrink (§6.1). Expect ~0.1–0.85M of fix-round build spend per finding-carrying round, reported inside the build number.

**Un-started cards keep their letter notation; read it through this table:** `S/M verify` → 1 standard pass · `L verify` → trigger-tier at 2–3 rounds · any story hitting the prompt-assembly or safety-gate class → 4–6 rounds regardless of the letter on the card. Restating every card was considered and rejected — the letters still rank stories correctly; only their price changed.

**Pipeline overhead (named 2026-08-28; run-story step 10 already reports the fourth number):** intake ground-truthing ~0.05–0.8M (scales with board staleness, not story size — E9-S1's whole blocked run cost ~55k of intake) · PLAN ~0.2–0.5M on M+ stories · QA ~0.1–0.3M · postmortem CAPTURE ~0.1–0.3M (E9-S1's 1.56M included a one-off charter adjudication; CURATE stays unpriced until its first firing) · process-rule design red-teams ~2–3.5M when a postmortem proposes charter/skill changes (E2-S2: ~3.4M). Stories with no verification pass carry a build band only.
...
| Token spend drifts above bands | **Realized, then repriced** | Per-run actuals reported (§6.5); bands recalibrated 2026-08-28 with verification moved to a rounds×unit model (§5) after six runs blew letter-priced verify bands by up to ~35×; next recalibration at the Arch v2 GO gate; XL work (E8-S4) sits behind the gate where the bet is already validated. |
