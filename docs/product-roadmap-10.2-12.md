...
**Size bands (output tokens): S** ≤150k · **M** 150–500k · **L** 0.5–1.5M · **XL** >1.5M.

**Bands cover build only; verification is budgeted separately** (approved 2026-08-24), written as `Size <build> build + <verify> verify` — e.g. `M build + L verify`. Trigger-tier adversarial verification cost scales with the number of *checkable claims* a story produces, not with the size of its diff, and read-only does not mean cheap: the cost sits in independent-lane fan-out and skeptic passes rather than in edits. Two calibration data. **E1-S1** was banded S–M and cost **~1.3M**, because a small diff on a safety gate still earns a full multi-lens review. **E2-S1** was banded S–M and cost **~5.1M**: four independent code-trace lanes plus an empirical marker-string harness, then three adversarial lenses with two independent skeptics across 26 candidate findings (16 confirmed / 5 contested / 5 refuted — see the Method note in `docs/prompt-injection-audit.md`). One blended band cannot represent either. Stories with no verification pass carry a build band only.
...
Bands are planning estimates — calibrate against 10.2 actuals and report **build, verification, and plan-absorption spend as three separate numbers** after each run (§6.5).
...
#### E2-S2 · Token breakdown visualization — **Kanban: Ready for Dev (S1 delivered 2026-08-24 via #449; spec = audit §8)** · Size **L build + L verify** (trigger-tier adversarial review budgeted separately from the build band)
...
#### E4-S0 · Client activation-engine repair — Data Bank + WI timers — **Kanban: Ready for Dev (no E3-S1 dependency)** · Size **M build + L verify** · Scheduled 10.3
...
| E9-S3 | Additional cloud providers (old "10.2": DeepSeek, Cohere, …). **Each provider must also land a `profileForProvider` case** (`tokenizer.ts:33-46`) — an unmapped provider silently falls through to `generic` (3.8 chars/token), which over-prices history and shrinks that provider's usable context window with no visible error. Note the ordering trap: adding the case is also what first exposes that provider to the `computeRagBoundary` divergence (the simulation omits the profile arg and is hard-coded `generic`, `ragBoundary.ts:122-130`; audit §5 = ~5% under-exclusion, recall can return a message already in raw history), so land this after — or together with — E2-S2 task 1b's boundary fix (#457) | Backlog | Haiku/Sonnet per provider — the API wiring is the pattern that exists; the tokenizer mapping is not | S each |
...
| E9-S6 | **Group macro substitution + blank-user-turn guard** (#451): group pushes card fields (`chatStore.ts:1938-1948`), author's note (`:2053`, overflow splice `:2087`) and history turns (`:2065-2071`) raw, so `{{user}}`/`{{char}}`/`{{setvar::…}}` ship as literal text; solo guards a blank user turn at `:1524-1539`, group does not. **Sequence ahead of E2-S2, not concurrent with it** — E2-S2's golden-prompt test asserts zero diff *before vs after instrumentation*, so a group-output change landing mid-story makes the golden diff unable to separate instrumentation drift from the intended fix, destroying the safety net E2-S2 task 2 exists to provide | Ready for Dev (schedule 10.3, ahead of E2-S2) | Sonnet impl + Haiku tests — small, isolated, loud failure | S–M build + S verify (verify budgeted separately) |
...
5. **Token reporting — build, verification, and plan absorption as three numbers.** After each phase: actual per-story spend vs the §5 band, one line each, with **build, verification, and plan-absorption spend reported separately**. A story that lands on-band for build and 3× over on verification is a verification-scoping problem, not a build-estimation problem; one blended number hides which, and the wrong lever gets pulled at recalibration. Plan absorption (§5) is reported against the story that *triggered* it, not against the stories it edits, so the true cost of a knowledge-producing story stays visible. Bands recalibrate after Phase 10.2 actuals.
6. **Verification is budgeted per checkable claim, not per story size.** §5's bands cover **build spend only**; when a story hits the §6.1 trigger list, its verification budget is set separately, from the number of claims a reviewer must check — findings produced, gate predicates, sections instrumented, migration fields, lenses required — not from the diff size. Corollary: **audit and design-verification stories band L minimum** (§5), because their entire output *is* a claim set. Calibration data: E1-S1 (originally banded S–M, ~1.3M actual, trigger-tier review dominant) and E2-S1 (originally banded S–M, ~5.1M actual; 26 candidate findings — 16 confirmed / 5 contested / 5 refuted — each fact-checked by three adversarial lenses with two independent skeptics). A verification budget is never traded away to keep a story inside its build band — the story slips, the review does not shrink (§6.1).
...
1. **E4-S0** (#450 High + #452) and **E9-S6** (#451) go first in 10.3 — both change emitted prompt content and must precede E2-S2's goldens (§7's ordering rule).
