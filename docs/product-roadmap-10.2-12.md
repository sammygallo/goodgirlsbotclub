...
| Trigger-tier, contained seam | §6.1 trigger, single-repo, no gate/contract/assembly surface | 1 full + 1 confirmation | ~1.7–3.5M | E1-S1: ~1.3M |
| Safety gate / backend contract | touches a §6.8 gate or a cross-repo contract | 2–3 full + confirmation | ~3.2–9.5M | E4-S0: ~5.0M |
| Prompt assembly / new mechanism over a frozen layer | touches either builder's emission, the dispatch seams, or ships new UI over a frozen data layer — **E2-S3 is in this class** | 4–6 full + confirmation | ~6.2–18.5M | E9-S6: 9.18M (4 rounds, 1 loop) · E2-S2: ~36.7M story-total over 4 loops (~17 rounds; the final loop's 5 full + 1 scoped ≈ 11M) |
| Claim-set / design red-team | the deliverable IS a claim set: audits, pre-code red-teams, process-rule changes (§5's L-minimum rule) | 1–2 adversarial passes over every claim | ~3.5–9.5M | E2-S1 audit: ~5.1M · charter v1.2 red-team: ~9.1M across 2 rounds (the §4.4 calibration datum) · E2-S2's D/E/F red-team: ~3.4M · this recalibration's own: ~4.6M |

The loop runs to convergence — the final round confirms zero — and a verification budget is never traded away to keep a story in band: the story slips, the review does not shrink (§6.1).
...
| Token spend drifts above bands | **Realized, then repriced** | Per-run actuals reported (§6.5); bands recalibrated 2026-08-28 with verification moved to a rounds×unit model (§5) after six runs blew letter-priced verify bands by more than an order of magnitude (worst on record: E9-S6 and E2-S2, each >20× their verify letter); next recalibration at the Arch v2 GO gate; XL work (E8-S4) sits behind the gate where the bet is already validated. |
