...
| (Not in handoff — found 2026-08-26 during E9-S1 intake) | "E9-S1 Phase 5.3 swap-vs-join card handling" carried into this roadmap as Ready for Dev. **It shipped 2026-04-11 in [PR #59](https://github.com/sammygallo/goodgirlsbotclub/pull/59)** (`b5938390`), an ancestor of `main` and of every image deployed since: `GroupCardMode = 'swap' \| 'join'` (`chatStore.ts:172`), the `cardMode` field on `GroupChatInfo` (`:200`) with legacy-chat migration defaulting to `'swap'` (`:346`), the two-branch card block in `buildGroupConversationContext` (`:2052-2087`), `setGroupCardMode` (`:3545`), and the "Card mode" segmented picker (`GroupChatControls.tsx:259-287`). E9-S6 added ~10 join-mode assertions to `chatStore.groupMacros.test.ts` last week (63/63 green). The claim had **four carriers**, not one — naming a single source was itself an error this correction originally made: memory `project_roadmap_status.md` row 5.3 (audited **the same day #59 merged**, so it caught the pre-merge tree), the tracked repo file `ROADMAP.md` §5.3 ("Gap: … No join mode"), this roadmap's E9-S1 row, and the untracked `HANDOFF_FABLE_GGBC_PRODUCT_PLAN.md`, which recommends Phase 5.3 as filler. A cold session greps the repo before it reads the memory dir, so `ROADMAP.md` was as live a trap as the memory row. All tracked carriers corrected 2026-08-26; the untracked HANDOFF doc corrected 2026-08-28 by an end-to-end sweep of every status row (superseded banner + 12 dead carriers fixed on disk, incl. Phase 10.1's void merge instructions and its false "gates 10.2" dependency). | Same class as the 10.1/6.1/E9-S4 corrections — a stale carry-over, not work. **E9-S1 removed from the plan** (§5's E9 table, §7 Deployed-pre-roadmap); memory corrected. One sub-clause of the *old* `ROADMAP.md` §5.3 wording — "join mode with **configurable** prefix/suffix" — was never built: the `## ` header, the `[SPEAKING NOW]` speaker marker and the `\n\n---\n\n` separator are hard-coded (`chatStore.ts:2062-2073`). The v3 row never asked for it, so it is **not** a residual story; recorded as an observation only. If it is ever wanted, note it edits the group builder and so inherits §7's ordering rule against E2-S2's un-seeded goldens. |
| (Not in handoff — found 2026-08-30 during E2-S5 intake) | "E2-S5 WI telemetry id crosswalk" rests on the claim that `captureWiFired` drops every server-path firing because backend UUIDs are "permanently disjoint" from local `wi_`/`wibook_` ids. **That was true when the in-code comment it quotes was written — `9ced3e43`, 2026-08-06 — and false the very next day.** The native-CRUD cutover (`77e689d2`, 2026-08-07 — the commit `docs/legacy-id-strip-scoping.md` itself calls "the 2026-08-07 cutover commit", and which introduced `normalizeNativeBook` and `set({ books: nativeBooks })` together) made native `/lorebooks` the system of record: `books` is populated *only* from that fetch and never from the blob (`worldInfoStore.ts:1938-1947`), `normalizeNativeBook`/`normalizeNativeEntry` adopt the server's primary key verbatim (`id: String(dto.id)`, `:2144`/`:2102`), and the backend activation engine ranks those same `lorebook_entries` rows (`_activation.py:77`). So the filter compares server UUIDs against server UUIDs and **passes**. Both halves are pinned by committed tests that are green on `main` today — `serverRetrieval.test.ts:308` (`'srv-entry-1'`) and `worldInfoStore.test.ts:1391` (`'native-entry-1'`). | **E2-S5 re-scoped, not removed** — unlike the 10.1/6.1/E9-S4/E9-S1 corrections, this card had three acceptance criteria and only the first is dead. AC1 is satisfied by the cutover; the crosswalk task is a no-op. AC2/AC3 survive under a corrected frame: pre-cutover `wi_fired` maps were never remapped (`chatStore` never calls the `remapLegacyEntryId`/`resolveLegacyEntryId` that `worldInfoStore` exports), and `wiReplay`'s `neverFired` (`wiReplay.ts:169`) still cannot distinguish "never fired" from "not observable". **E5-S1's gate stands.** The origin comment is corrected in the same PR, so the dead premise cannot be re-derived from code — the failure mode here was a true comment outliving the architecture it described, with the roadmap card quoting it verbatim 24 days later. |
...
**3. Transparency before optimization.** The Optimization Agent (E5) needs two different things to make a claim like E5-S1's own worked example — "entry X: 0 triggers in last 25 chats, 450 tokens/turn when active." The **token half** comes from Token Breakdown (E2-S2), whose section taxonomy is the audit's §8 (approved). The **activation half** — which entry fired, on which turns, under which engine, and whether it was budget-evicted — exists only in part today, and this chain does not provision the rest: the server activation engine computes a per-entry reason internally (`_activation.py:228` — constant/keyword/semantic/sticky), **which E2-S2a shipped through to the client on 2026-08-29**; `captureWiFired` **does** record server-path firings today — the two id schemes converged at the 2026-08-07 native-CRUD cutover (`77e689d2`), so the "no crosswalk" claim this bullet used to carry is dead (§1) — but pre-cutover telemetry keyed on `wi_`/`wibook_` ids was never remapped and `wiReplay` still cannot report "not observable"; and `wiScanReport`'s budget-eviction list stays zeroed on server-path turns (`chatStore.ts:1304`). The audit's §6 names the client-side fired-WI telemetry and `wiScanReport` as the hooks to build on — the work is closing those three gaps, not starting from nothing. So the real order is: audit → breakdown **+ activation-reason plumbing (E2-S2a, shipped) and telemetry coverage (E2-S5)** → insights data API (E2-S4) → agent. The audit itself was neither cheap nor read-only — it needed a worktree-local instrumentation seam (audit §8) and is re-banded **L** under the approved audit/design-verification minimum; it completed in 10.2, producing the §8 taxonomy and issues #450–#456.
...
flowchart TB
    ...
    subgraph TRANSPARENCY["Track E2→E5 · Transparency → Optimization"]
        ...
        E2S5["E2-S5 WI telemetry coverage"] --> E5S1
...
| **E2-S5** WI telemetry coverage | Transparency | Pre-cutover `wi_fired` maps remapped or flagged partial-coverage, and `wiReplay` able to say "not observable" rather than "never fired" — hard-blocks E5-S1. **Re-scoped 2026-08-30:** the original id-crosswalk task is a no-op since the 2026-08-07 cutover (§1) |
...
#### E2-S5 · WI telemetry coverage — **Kanban: Backlog** · Size S–M · **hard-blocks E5-S1, feeds E2-S2's drill-down**
**Re-scoped 2026-08-30 at intake; the original "id crosswalk" premise is dead — see §1.** The card was written from a `captureWiFired` comment dated 2026-08-06 asserting backend UUIDs are permanently disjoint from local `wi_`/`wibook_` ids. The 2026-08-07 native-CRUD cutover (`77e689d2`) made native `/lorebooks` the system of record and collapsed the two schemes, so a server-path firing is **already** recorded under the local entry's id and the crosswalk task is a no-op. That comment is corrected in-tree so the dead premise cannot be re-derived from code.

What remains is telemetry *coverage*, in two independent gaps. **(1) Pre-cutover telemetry is orphaned.** `wi_fired` maps persisted in chat headers before 2026-08-07 are keyed on `wi_`/`wibook_` ids and nothing remaps them: `worldInfoStore` exports `remapLegacyEntryId`/`resolveLegacyEntryId` for precisely this and `chatStore` calls neither, so historical firings silently read as never-fired — the exact false signal E5-S1 must not build an "unused entry" finding on. **(2) `wiReplay` cannot say "not observable".** `neverFired` is active entries with no `fired[key]` (`wiReplay.ts:169`), which collapses "we watched and it never fired" into the same value as "we hold no telemetry for this chat at all"; `approximate` is a single boolean over the whole map and cannot carry the distinction either.

Note the *live* coverage gap is now narrow and different in kind from the one this card used to describe: `captureWiFired` still drops entries the local store cannot resolve — demonstrably, an entry created on another device since this client last ran `fetchPrefs` — which is a sync-window gap, not a scheme mismatch, and is not in scope here. It is **not** the shared-book case, in the opposite direction to the obvious guess: an active shared-origin book makes a chat ineligible for server retrieval outright (`serverRetrieval.ts` condition 4b), so the server path never activates over one.

Sonnet — the remap is mechanical and the observability state is a small type change. Opus consult only if (1) proves to need a persisted migration rather than a read-time remap; **read-time is strongly preferred**, since it is reversible and `wi_fired` is telemetry rather than user content.

**Acceptance:** pre-cutover `wi_fired` keys resolve to their current native ids, or the map is explicitly flagged partial-coverage rather than read as low trigger counts · `wiReplay`'s output distinguishes "never fired" from "not observable", and `approximate` is not overloaded to carry both · **a regression test pins that a server-path firing IS recorded under the local entry id**, so the falsified premise cannot silently return · no change to what `captureWiFired` persists for entries the local store can already resolve.
---
...
| # | Task | Tier — reasoning |
|---|---|---|
| 1 | Heuristics design, **written against the audit's activation ground truth** — unused WI (not triggered in N recent chats), redundant lore, oversized prompts, budget misalignment, with thresholds + severity levels. (a) "Unused WI" must name its evidence source and its coverage. Two substrates exist and they are biased in opposite directions — the fired-WI telemetry (`chatStore.ts:1709-1739`) records what actually reached the prompt (`matchedEntries = serverMatchedEntries ?? scanMessagesForEntries(...)`, `:1071-1094`), while the persisted per-chat timer record (`worldInfoStore.ts:1010`) records activation *before* the budget trim and therefore over-reports. Reconcile the two; do not trust either alone. (b) **Never-fired ≠ unused.** Keyless `semanticOnly` entries (Data Bank document chunks) have no keyword path in the local scan — `entryMatchCount` returns 0 for any keyless entry (`worldInfoStore.ts:1246`) — and fire instead through the server engine's semantic/FTS legs (`_activation.py:485`, `sql_hit = entry.id in semantic_scores or entry.id in fts_scores`, gated for keyless/`semanticOnly` entries at `:495-499`). A zero-trigger count is a capability gap, not waste (audit §3b, F1/#450): exclude them from the unused-WI heuristic, or report them under a distinct "can't activate in this configuration" class — mirroring the exemption `lorebookLint.ts:113-120` already makes (which also exempts `constant` entries, since those fire regardless of keys). (c) Down-rank any zero-trigger finding that a known bug explains better than real disuse: **#450** — Data Bank chunks cannot fire on local-scan turns at all (§3a/F1); **#452** — an entry evicted by the WI budget still advances its own cooldown/sticky timer and so refuses to fire the next turn. #452 is **not client-engine-only** as audit §3b's closing line states: the backend ported both halves deliberately, and **E4-S0 has since fixed both** (shipped 2026-08-26, `4196478a`) — so read this row against the post-E4-S0 engine, not the audit's snapshot. `ActivationResult`'s docstring (`_activation.py:374-406`) now records that `activated_entry_ids` *used to be* captured before the budget trim and is now the post-trim intersection; and sticky carry-overs no longer bypass group resolution — `:563-610` admits them through the **same** one-winner-per-group constraint fresh matches went through, with a deliberately deterministic tie-break pinned as a two-engine contract. **Both citations this row previously carried were wrong subjects, not line drift** (verified 2026-08-29 during E2-S2a): `:519-538` is RRF rank-fusion and `:466-468` is the timed-effects filter loop. (d) "Budget misalignment" must reconcile the WI budget's raw-content accounting against emitted bytes — the budget is measured on raw stored content, before macro expansion and before `wrapWiContent`'s attribution lines, so it systematically undercounts emitted tokens (audit §4.1) — and must handle `wiScanReport` being zeroed on server-path turns: derive evictions there from `activated_entry_ids` − returned `entries`, or mark those turns unobservable. (e) Every finding carries a coverage statement ("observed on N of M turns; K turns unobservable"); no finding may be emitted at warning level or above from unobserved data. | Opus — heuristic quality is the product; over-aggressive recommendations poison trust, and several independent mechanisms can make a live entry look dead |
