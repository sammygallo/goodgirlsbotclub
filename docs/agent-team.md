...
**Prepared:** 2026-08-24 · **Updated:** 2026-09-10 (v1.3) · **Status:** ✅ **Approved by Sammy 2026-08-24 (v1.1)** — the §6 files are scaffolded and this is the team's operating charter *(v1.1 = self-critique + wheel-check pass; see design-review notes after §9. v1.2 = the `postmortem` role, added after both pilots' retros were hand-copied into four unsynced places apiece; red-teamed pre-merge under §4.4 and revised on its 7 confirmed findings — see §10. v1.3 = seven operating rules, §4 items 8–14, from the 2026-09-10 charter-and-ledger efficiency review, Sammy-directed; figures and the companion edits they owe are in §11.)*
...
- **v1.3 change (2026-09-10):** seven operating rules (§4 items 8–14) from a charter-and-ledger efficiency review. The ledger showed build at roughly 4% of recorded spend, governance plus postmortem at roughly as much as verification, review yield falling on the recent frontend loops, and the PM reading ~100k tokens of rule text per run. The rules: report yield with the actuals · review agents never receive the shared checkout · the claim surface is budgeted at the source · the PM's numbers are derived by script, never read from prose · rules and their evidence live apart · a round card after every review round · cards and index lines carry current state only. Figures in §11.
...
- **Contract before handing back — deterministic self-gates, all green:** full test suite, **`npm run build`** (PR CI doesn't typecheck tests — the house's known trap), lint from a clean checkout. Returns a structured report: files touched, tests added, deviations from plan with reasons. Comments in new code state intent and carry no checkable specific a test does not already pin (§4 item 10).
...
- **Trigger tier (L/XL or any trigger-list hit):** the bespoke `story-review` workflow — the proven house org: N independent lenses → skeptic verification of each finding → fix-on-branch → re-verify, plus **mutation-verified kill tests** for anything touching a safety gate. Lenses and skeptics alike run on the `adversarial-reviewer` **stance** — `.claude/agents/adversarial-reviewer.md`, condensed into an inline constant in `story-review.js` because that script must also run where custom agent types are not loaded (its own comment; #517 re-synced the pair) — lenses at opus/high, skeptics at high effort on the model the PM's session resolves to (`story-review.js:177` pins none: inherited, not guaranteed strongest, so switching the session model mid-review switches the skeptics too — run the trigger tier from the strongest tier): **verifiers are never economized**; a weak verifier is negative value. The agent *type* itself is what the PM spawns only for one-off lenses outside the workflow (the ad-hoc red-team of a PR or a proposal); the design red-teams this charter mandates — a roster design doc, or a proposal that changes how the team works — run `story-review` in design mode, on that same inline stance. This tier earned its 11x-confirmed record on exactly these story shapes; it is reserved for them, not spent on everything.
- **Timing is the point:** review runs on the branch, **before** the PR is marked ready — never alongside, never after (Phase 5 shipped 9 known defects the other way). Every finding ends CONFIRMED-fixed, REFUTED-with-reason, FILED-as-issue (pre-existing-and-adjacent only, P10 — issue link in the bundle), or ACCEPTED-with-reason in the evidence bundle.
- **Reviewers never receive the shared checkout (§4 item 9):** every lens and skeptic works on a detached copy of the branch at its sha and is never told the shared tree's path; mutation-testing a coverage claim happens inside that copy, which is removed after the round, so the target stays byte-identical to its committed state. The instruction-only form of this rule was violated five times — pilot E1-S1's stray mutation, then E2-S2, E9-S9, E2-S4 PR1 and #528.
...
- **Why it exists:** the team is ephemeral and its documents are its only memory (§4.3) — but capture was one unenforced line in the DoD, run by the most exhausted context in the pipeline. It shows. The E1-S1 worktree-mutation lesson lives in **four** copies (`story-review.js`'s executable stance string, charter §2 above, a `MEMORY.md` index line, and a memory-file body), three of them unsynced — the stance string has been generated from the agent file since 2026-09-09; the E2-S1 dead-skeptics lesson has the same four. `feedback_workflow_empty_result_check_failures.md` was written only *after* that bug scored 21 unverified findings as confirmed. And memory has grown to a size where the always-loaded index is itself a cost, with at least one file grown by appending rather than distilling.
...
- **CAPTURE (every exit):** diff what happened against what the charter predicted — band vs actuals per category, whether the brief's gotchas fired, what bit that no document covered, whether a *rule* proved wrong (which outranks a wrong fact: a bad rule misfires on every future story). Routing a lesson includes routing it **completely** — for the `adversarial-reviewer.md` / `story-review.js` stance pair that now means naming the **agent file plus the regeneration step**, since the constant is generated from it and gate-checked (`postmortem.md` §A5); it was a hand-synced duplicate, and drifted, until 2026-09-09.
- **CURATE (when the ledger shows ≥5 exits since the last CURATE, or on request — the only two triggers):** refute-first staleness audit — mechanical sweep of every named file/symbol/PR, index-vs-body drift, accretion (**repetition, not length**), duplication. **A stale claim is remedied by deletion at minimum span, never replacement prose** — accretion keeps §B3's restructuring remedy — and a sweep shrinks memory or holds it, never grows it (`postmortem.md` §B). Off the per-story clock deliberately: staleness accrues with repo change and calendar time, not with story closes, and the sweep is most of this stage's cost. The count is **read from `docs/agent-team-log.md` by the ledger script (§4 item 11), never remembered** — a fresh PM session has no cross-run state, so a cadence that isn't computable from a committed file is one that silently never fires. Its extent is **reported, not assumed** (`entries_swept / entries_total` + the commands run) — a partial sweep is fine, a partial sweep reported as complete is not. CURATE also restores `MEMORY.md` lines that grew past one-line hooks and roadmap cards that accreted correction history (§4 item 14), and its ledger row states the window's spend split (§4 item 8).
...
- **Proposes; never writes — and the gate keys on `removes_content`, not on kind** (§5). A full-body "update" of an unversioned file destroys exactly as much as a delete, so blast radius is the only honest partition; any such proposal carries a `preserved[]` list and the PM snapshots the pre-image before applying. Doc edits ride the normal PR gate, and any proposal changing *how the team works* is red-teamed by `story-review` in design mode first — **batched into one governance release per CURATE cadence rather than one red-team per proposal**, because serial passes over small edits mostly find what the previous fix introduced (#523: five of pass 2's seven confirmed findings were in the mirror pass 1 had just added). Mechanics and the expedite clause for an actively-misfiring rule live in `run-story` §10c, not here.
...
**Definition of Ready** (gate into BUILD): AC testable as written · dependencies verified against ground truth, not the board · design doc approved where the roadmap requires one · brief compiled · each AC paired with the cheapest evidence that satisfies it (§4 item 10) · tier map printed.
...
**Design-story variant** — the roster is **E1-S2, E3-S1, E8-S1, E8-S3**; the mechanics live in exactly one place, `run-story`'s *Design-story variant* section (§4.6, thin agents / fat skill), and this list is the roster, not the procedure. The deliverable is a Sammy-approved doc in `docs/`. A story on this roster runs an adversarial pass whether or not its own task table names one, so **it bands L minimum** and prices off roadmap §5's claim-set row — **the roster and that band are changed together, never separately.** *(E6-S1 was removed from the roster 2026-09-04: a mockup workshop's gate genuinely is Sammy's approval and a red-team has no claim surface to attack there. It keeps its build band and still takes design sign-off under §5. Both copies of the mechanics had also drifted before the pointer above existed — the charter said "steps 3–5 … step 8 is n/a" and the skill said "steps 4–6 … step 9 is n/a", right and wrong in opposite halves, found at E8-S1 intake 2026-09-03.)* Security-relevant designs are red-teamed **pre-code** — the practice that caught the Phase 3 scope hole.
...
7. **Session concurrency — item 1's WIP limit governs STORIES; this governs SESSIONS, and they are different axes.** Sammy runs several Claude sessions against this repo at once, and they collide in ways no story-level limit catches, because that limit counts open story PRs while the damage comes from two sessions touching one working tree. Evidence, all from this repo: **six extra worktrees** on 2026-08-30, the oldest 28 days, two of them left inside a *different session's* scratchpad path; a derelict worktree found 2026-08-30 holding uncommitted dependency churn, and another on 2026-08-29 holding a live uncommitted mutation that **deleted the World Info dedup guard**; and concurrent pytest against one shared postgres corrupting results **three times in a single run** (E2-S2a), each time presenting as ~368 failures at *setup* in files nobody had touched.

   Four obligations, cheapest first.

   a. **Name your peers at INTAKE.** `ListAgents` lists the live sessions on this machine. Report every *interactive* peer it shows, with the branch you are about to take — do not try to decide which of them are on this repo, because the listing names sessions and does not report their working directory. Sammy is the only one who knows what those sessions are doing, and he cannot warn you about a collision you never mentioned.

   b. **Treat the brief's `FILE / SEAM POINTERS` field as the collision surface, and print it before BUILD.** It already exists in `run-story` §2 — this adds no new field, only the reason it is load-bearing and the deadline. A collision is cheap while it is still a plan and expensive once both sides hold commits.

   c. **A worktree belongs to the session that created it, and that session removes it at close.** When someone else's worktree blocks the tree-hygiene gate, resolve the *narrowest* thing that clears it and report what you did: restoring a regenerable build artifact (a dependency cache, a lockfile the tooling rewrites) needs no permission, because nothing authored is at risk. **Anything that could destroy authored work — removing the worktree, resetting it, or reverting a source file — needs Sammy**, and a dirty worktree is far likelier to be somebody's live work than litter. The 2026-08-29 derelict is the case in point: it looked like litter and held a deleted safety guard. **This can legitimately stall the pipeline** — a dirty source file in someone else's tree blocks the hygiene gate and you may not clear it yourself. Stalling is the correct outcome; report it and wait rather than improvising a way around the gate.

   d. **Never judge a worktree abandoned with `git merge-base --is-ancestor`.** `/deploy-ggbc` step 2 mandates squash merges, which give the merged work a *new* sha, so the original branch head is not an ancestor of `main` and reads as UNMERGED even though it shipped. Worked example, from this pipeline's own merge on 2026-08-30: PR #496's head `248c8fd9` answers **NO** to `--is-ancestor origin/main`, while `gh pr view` reports it **MERGED** and its `src/stores/chatStore.ts` is byte-identical to `main`'s. Five of this repo's six extra worktrees read UNMERGED under that test the same day, which tells you nothing about whether any of them shipped. **Judge by content (`git diff <sha> origin/main -- <file>`) or by `gh pr view`, never by ancestry** — and note the trap runs deeper than it looks: an ancestry check can also read *stale-but-superseded* work as live, so a NO means "ask", not "abandoned". (This is the same defect that made `/deploy-ggbc`'s notify loop blind for its entire life: ancestry and squash merges do not mix.)

   **Test isolation is not optional and is not restated here:** the `project_ggbc_dev_env` memory carries the per-run postgres+network recipe, and it is per-run precisely because a shared pair corrupts concurrent runs. Follow it rather than reinventing it.

8. **Review yield is reported with the actuals.** Per review loop, beside the round count: production-behavior defects closed CONFIRMED-fixed ÷ verification tokens. A behavior defect is a finding whose failure scenario is a runtime outcome; a false comment, docstring or test name is a real finding and a real fix, but it does not count here. The figure goes in the merge notice and the ledger row, and every CURATE row states the window's spend split — build / verification / governance + postmortem — read off the ledger. Yield is information, never a cap: it tells the recalibration which instrument to point where. *(v1.3: the four most recent frontend code loops — E2-S5 #500, E9-S7, E9-S9, E2-S4 PR1 — spent ~70M of verification for three behavior defects fixed, and nobody had computed it because nothing required it.)*

9. **Review agents never receive the shared checkout.** Every lens and skeptic works on a detached copy of the branch at its sha, created for the round and removed after it, and is never told the shared tree's path; the copy is what the workflow's diff target names. Until `story-review.js` creates the copy itself, the PM creates it and passes that path. After every round the shared tree's HEAD and porcelain are checked unchanged — `tree-hygiene.sh` sees dirt, not a moved HEAD. *(v1.3: the instruction-only rule failed five times; CURATE round B's sandbox — copies, path never disclosed — produced zero live writes.)*

10. **The claim surface is budgeted at the source.** Review prices every checkable sentence in the diff (D-T4), so the diff carries only the checkable sentences it needs. Comments in new code state intent and carry no specific a test does not already pin — no counts, no `file:line`, no *only / always / never / last*, no attribution. `story-dev`'s "match the surrounding comment density" reads as density of intent comments, never of claims. A fix round that finds a false comment deletes it and writes none. The same budget covers test tooling: at BRIEF each AC is paired with the cheapest evidence that satisfies it, and a guard that grows into a mechanism of its own is a scope decision under item 4, not a fix round. *(v1.3: E2-S4 PR2's type file is about two-thirds comment lines, and nine of its 25 commits hardened a 768-line import scanner built to prove one AC.)*

11. **Numbers the PM needs are derived, never read from prose.** The CURATE counter, the exit list since the last CURATE row, and the paths recorded for `.pending/` and `.pre-image/` come from a tested script over the ledger's leading columns (`.claude/scripts/ledger-index.sh`, same test obligation as `tree-hygiene.sh`). The PM reads the script's output; the ledger's rows are evidence, read when a row is the question. *(v1.3: the ledger is 104KB and step 10c required reading it whole to compute a count of five; the count was missed once, at E2-S5, and hand-reconciled on every run since.)*

12. **Rules and their evidence live apart.** In `run-story` and in §2–§5 here, a rule is one or two imperative sentences plus a pointer — the ledger row by date and story, or the PR — and the incident narrative lives at the pointer, not in the rule. The skill lint's size floors sit at a fraction of each section's size, re-based per governance release, so a trim is not itself a governance event while a wholesale deletion still fails. Every governance release states the sentences it adds and removes from the two critical-path files. *(v1.3: `run-story` grew from 6.6KB to 61.5KB in sixteen days and carries 45 story-id citations; its floors were the exact current sizes with no slack, so growth was free and every deletion was a red-teamed PR.)*

13. **A round card after every review round, outside the subject.** When a round closes, the PM appends one line — round number, subject range, confirmed / plausible / refuted / unverified, closures, the Q1/Q2 answer — to `<memory-dir>/.runs/<story-id>-<YYYY-MM-DDTHHMMSS>.md`, beside `.pending/`, never on the story branch and never a review subject. The bundle's per-round lines are copied from it at §7, the postmortem receives it as the run record instead of a transcript, and it is readable mid-flight. A loop with no card for its latest round has skipped the card, not the round. *(v1.3: E2-S4 PR2 stood at 14 rounds and 25 commits with no PR, a board card still reading *build* from PLAN exit, and no record anyone could read.)*

14. **Cards and index lines carry current state only.** A roadmap card says what is true now; a correction deletes the false clause at minimum span, and the history of the correction lives in git and the ledger, not inline. `MEMORY.md` lines are one-line hooks; a line that has grown into a paragraph is restored by the next CURATE, and the content it carried lives in its file or nowhere. *(v1.3: the E2-S4 card carries corrections of its own corrections, and several index lines run to hundreds of words.)*
...
| `.claude/scripts/ledger-index.sh` | Derives the CURATE counter, the exit list and the recorded `.pending/` / `.pre-image/` paths from the ledger's leading columns; tested like `tree-hygiene.sh` (**v1.3**, owed — not yet built) | S |
...
## 11 · Efficiency review notes (v1.3, 2026-09-10)

Sammy-directed review of this charter, `run-story`, the agent files and the ledger, sixteen days after launch. Figures are read off the ledger's rows and carry its caveat: estimates, not meter readings; PM main-loop tokens are excluded everywhere.

| Category (2026-08-24 → 2026-09-10) | Approx. tokens | Share |
|---|---|---|
| Build | ~14M | 4% |
| Verification (story loops + QA) | ~160M | 49% |
| Plan absorption | ~13M | 4% |
| Postmortem stage | ~31M | 9% |
| Governance red-teams, CURATE batches, bookkeeping | ~108M | 33% |

Merged PRs in the window: 52 — 14 touching product code, 26 governance-only, 12 docs/board/roadmap only. Per-run PM reading before any story work: ~100k tokens (`run-story` 61KB, this charter 44KB, the ledger 104KB, the memory index 20KB, plus the roadmap card).

**What each rule answers.** Item 8: verification yield had fallen without anyone measuring it. Item 9: reviewer isolation as instruction failed five times; as a sandbox it held. Item 10: the D-T4 doctrine and the house's comment density together manufacture the review surface the loop then pays to clear. Item 11: the counter the pipeline depends on cost a 26k-token read per run and was still missed once. Item 12: the skill lint's exact-size floors made rule text a ratchet. Item 13: a runaway loop was invisible until exit, which is the E9-S9 shape recurring. Item 14: cards and index lines had absorbed the ledger's never-rewrite doctrine and grown correction chains.

**Companion edits these rules owe, each a governance change under §5:** `run-story` §5 (round card; evidence-by-pointer restructure) and §10c (counter via script; spend split in CURATE rows); `.claude/agents/story-dev.md` (comment rule); `.claude/agents/adversarial-reviewer.md` plus its regeneration step (a copy, never the shared path); `story-review.js` (create the detached copy; emit the yield inputs); `.claude/scripts/skill-lint.sh` (fractional floors) and the new `ledger-index.sh`; `postmortem.md` §B (index hooks, card history, spend split). Not design-red-teamed at the time of writing; §10c's batching applies — one release, one red-team.

---

*Approved 2026-08-24; §6 files scaffolded the same day (`.claude/agents/`, `.claude/skills/run-story/`, `.claude/workflows/story-review.js`). Pilot 1 = E1-S1 (review/QA/deploy legs) and E2-S1 both shipped. **§7's "Pilot 2 = E9-S1" was retired 2026-08-26 — that story had already shipped in April; see §7 for what is and is not yet proven.** v1.2 (2026-08-26) added the `postmortem` role at `run-story` step 10c, red-teamed pre-merge under §4.4. v1.3 (2026-09-10) added §4 items 8–14 and §11 from the efficiency review; the companion edits it owes are listed in §11.*
