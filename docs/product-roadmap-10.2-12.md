...
Note the *live* coverage gap is now narrow and different in kind from the one this card used to describe: `captureWiFired` still drops entries the local store cannot resolve — a turn racing `fetchPrefs`, an entry created on another device since the last sync, a shared book absent from `sharedBooks` — which is a sync-window gap, not a scheme mismatch, and is not in scope here.
