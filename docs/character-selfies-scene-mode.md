...
4. **Gate Phase A on a real-avatar validation pass.** Before committing the hosted backend, validate nano-banana on a handful of *actual* GGBC character cards (some more illustrated, some more semi-real) — the prototype used a stand-in. This is a Phase-A prerequisite, not a fork.
   ...
   **→ RAN 2026-08-19 (model-level, adversarially judged): 1 pass / 3 marginal / 0 fail** across 4 real prod avatars spanning painterly-anime (300px ref) → semi-real anime (1536px) → painterly fantasy → near-photoreal (300px). Identity *recognizable* and art style held 4/4 (illustrated stayed illustrated, photoreal stayed photoreal); scene genuinely changed 4/4 — the thing Kontext can't do. The marginals are **signature-detail drift** (an iris-color transplant, a lost tear-track mark, a lost hair-over-one-eye cut, accessory hardware changes) — i.e. §2's "not pixel-perfect" cost, now measured; the premium LoRA tier (decision 3) is the fix for signature-trait-perfect Scene. Operational findings: (a) hosted **input moderation** rejected one suggestive-wording × suggestive-avatar combo (`SEXUALLY_EXPLICIT` multimodal flag); neutral rewording of the same request passed — expect the same class of 4xx on fal, surfaced via the existing error toast; (b) the single-subject prompt anchor holds at the *subject* level but distant background figures still appear in street/beach scenes — reinforcing the Phase B output-side-gate precondition; (c) "full-body" framing under-delivers (shin/thigh crops), 4/4. Full per-character verdicts: [ggbc-backend #67 comment](https://github.com/sammygallo/ggbc-backend/pull/67#issuecomment-5349190481). Residual: one live smoke run of the *fal endpoint plumbing* on a real `api_key_fal` after deploy.
