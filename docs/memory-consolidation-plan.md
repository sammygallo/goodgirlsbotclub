...
**Status:** Draft v2 (adversarially reviewed against code, 2026-08-21). **Phase 0 done** (ggbc-backend@1d3af97, 2026-08-21). **Phase 1 done** (ggbc-backend@795f185, 2026-08-21) — migration + worker branch only, deliberately dark. **Phase 2 done** (ggbc-backend@8bcfac8 + goodgirlsbotclub@37da7edc, 2026-08-22) — enqueue hook, retrieval endpoint, and client cutover shipped together; server-side embedding is now live end-to-end (once deployed — see rollout note below). **Phase 3 mostly done** (ggbc-backend@6010155 + goodgirlsbotclub@dee9dd17, 2026-08-22): 3.1, 3.2 (backend only), 3.3's `stm_data_bank` exclusion + `getSettingsBlob` memoization, 3.4, and 3.5 shipped. **Two items explicitly NOT done, both flagged rather than silently skipped:** 3.2's frontend toast (deferred — see that section), and 3.3's `stm_worldinfo.books` strip, steps (a)/(b) (needs a new backend id-remap design; genuinely risky, see that section).
...
### 3.2 Pinned-budget guard (pickup item 2, finally) — backend done, frontend deferred

**Backend done — ggbc-backend@6010155 (2026-08-22).** `PinnedBudgetWarning` ships on `create_entry`/`update_entry` responses. **Frontend toast/dialog explicitly NOT done**: `createLorebookEntry`/`updateLorebookEntry` have four call sites in `worldInfoStore.ts` (interactive single-edit, two bulk blob-import loops, and `autoMemoryStore`'s background extraction) — firing a toast on the bulk-import or background-extraction paths would be noisy/wrong UX, and picking the right one(s) needs its own scoping pass rather than a guess. The field is additive and safe to leave unread client-side in the meantime.
...
### 3.3 Stop hydrating dead weight on app start — partially done
...
- **`stm_worldinfo`: the legacy `books` field is NOT safely strippable yet** (caught in review) — **still not done; needs its own pass, not a quick follow-up.** `worldInfoStore.fetchPrefs` reads `stored.books` as the input to `buildLegacyIdRemap` — the code calls it the *"cross-device source of truth for a fresh browser whose localStorage cache is empty"* (`worldInfoStore.ts:3378-3380, 3448-3452`) — and when the remap comes up empty with native fetches succeeding, unresolved legacy `wibook_`/`wi_` ids persisted in *other* synced sections (chat-linked books, persona/character links, chat-lore `excludedEntryIds`) are **confidently dropped**, destroying real user state on exactly the migrated cohort. Sequence it: **(a)** first persist the old→new id map server-side at import time (`import_lorebooks_from_blob` knows the pairing) into a compact section or the book rows' `extra`; **(b)** teach the remap to prefer the server-side map; **(c)** only then strip `books` from hydration, gated on the map existing. The section itself always stays — it is the live home of `activeBookIds`/`tokenBudget`/`scanDepth`/etc.
  - **Research findings (2026-08-22, not yet implemented):** `Lorebook` (the book-level ORM model) has **no `extra` JSONB column** — only `LorebookEntry` does. So "the book rows' `extra`" isn't available without a new migration; the **compact-section route is the lower-risk, lower-effort choice** for both book- and entry-level pairs (no migration, rides the existing `GET /sync/sections` response for free). `import_lorebooks_from_blob` already computes an entry-level `id_map` (old `wi_...` id → new UUID, `lorebooks.py:777-799`) but it's a per-book-iteration local variable, recomputed and discarded every call — persisting it needs a **batch-level accumulator merged/upserted (not overwritten) across repeated idempotent calls** to that endpoint, since a re-run only touches newly-imported books and would otherwise clobber pairs an earlier call recorded. `buildLegacyIdRemap`/`resolveLegacyBookId`/`resolveLegacyEntryId` and the four other stores that depend on the remap (`chatLoreConfigStore`, `characterStore`, `personaStore`, plus worldInfoStore's own `chatLinkedBookIds`/`activeBookIds`) are all mapped in detail in this session's research — re-run that research fresh rather than trusting this summary before implementing.
