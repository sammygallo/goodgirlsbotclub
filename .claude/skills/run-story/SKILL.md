...
- **After every review round, and again before QA: the tree must match its pre-review baseline.** Snapshot `git status --porcelain` in EVERY repo the story touches BEFORE the review stage starts; after every round, run it again and paste the diff-vs-baseline into the run record. ANY delta — a newly modified tracked file, a new untracked path — stops the pipeline until explained and removed. (Baseline-diff, not empty-output: a long-lived untracked local file is not a violation, and hiding one via .gitignore is forbidden — an ignored file drops out of ripgrep's default traversal, silently breaking the INTAKE/absorption carrier sweeps that must reach 'any handoff doc'.) This is the check the prose rule lacked when it failed twice (E1-S1's in-place mutation, which `git worktree list` cannot see; E2-S2's stray `skeptic-wt` worktree). `git worktree list` is the secondary sweep for stray registrations; a worktree under `.claude/worktrees/` that no live session owns is a finding, not an exemption.
