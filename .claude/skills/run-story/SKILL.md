...
- **Read the LIVE artifact immediately before every publish and build your publish from that** — `Artifact action:"read"` on the stored URL — never from your working copy or the committed file. Concurrent sessions each hold a different base, so a publish rendered from a working copy silently deletes whatever another session's in-flight story put on the board (near-miss 2026-08-29, #489: the platform's read-before-write guard caught it, which is luck, not a pipeline control).
...
- **Tree hygiene — before the review stage and after every round, in EVERY repo the story touches. Paste the output.** Three checks; each covers a hole the other two cannot see. All three were verified empirically 2026-08-29 — #486 records three earlier versions of this rule that read plausibly and did not work, so do not substitute a command without running it against a planted violation first.
  1. **`git status --porcelain --untracked-files=no` must be EMPTY** — zero modified tracked paths, every time, *including the pre-review run*. This is the E1-S1 check and it is content-safe by construction: an in-place edit always appears. Do not weaken it to a diff against a baseline — porcelain reports paths and status codes, never content, so a file already listed as ` M` can be mutated again with the diff staying empty. A dirty starting tree is the contested-checkout condition, to be resolved before review rather than baselined over. **§3's board refresh is a tracked edit — commit it on the story branch instead of leaving it dirty**, or it becomes a standing violation of this line.
  2. **`git status --porcelain -uall --ignored=traditional -- .claude/worktrees`** — enumerates stray checkouts. `.claude/.gitignore` hides `worktrees/` from plain porcelain, so this needs its own probe, and **`--ignored=matching` does not work**: it halts at the ignored directory and emits the constant `!! .claude/worktrees/` whatever is underneath, so it can never fail. `traditional` lists the actual paths.
  3. **`git worktree list`** — stray *registrations*. It is blind to a leftover directory from a failed remove (check 2 catches those), and blind to an in-place mutation (check 1 catches those). A worktree no live session owns is a finding, not an exemption.
  **Untracked files: enumerate the expected ones before review, with a `shasum` of each, and re-check list AND hashes after every round.** A new untracked path or a changed one stops the pipeline. The repo has a long-lived untracked file — the handoff doc — that INTAKE and plan absorption must be able to grep, so hashing is what guards its content. **Never silence a path with `.gitignore`**: an ignored file drops out of ripgrep's default traversal and silently breaks those carrier sweeps (tried and reverted, #486).
