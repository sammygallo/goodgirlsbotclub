...
| F6 | Low | Recall's `no_key` reason never surfaced to the user. *Fixed:* E9-S7 (closes #455) — the client now surfaces `no_key` as a once-per-session toast plus a console.warn on every degraded recall call. | [#455](https://github.com/sammygallo/goodgirlsbotclub/issues/455) |
...
**F6 — `reason: "no_key"` never surfaced.** The backend distinguishes "no embedding key configured" from "no matches", but the client discards the field (`chatStore.ts:925` region), so a user with recall enabled and no key sees silence. *Fixed:* E9-S7 (closes #455) — the client now surfaces `no_key` as a once-per-session toast plus a console.warn on every degraded recall call.
