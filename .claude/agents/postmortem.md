...
**The remedy for a stale claim is deletion at minimum span — never replacement prose.** Cut the clause or sentence that is false; soften an overclaim, or mark a claim whose premise has closed — past tense, `HISTORICAL`, or one clause naming what closed, carrying no new specific — only into something that will stay true; add no commit sha, PR or issue number, `file:line` cite, date, or count that is not already in the file. Every new specific is a new rot surface. The evidence is the 2026-09-02 sweep (ledger rows of 2026-09-05): its 261 `fix_text`s were freshly researched replacements and dated per-claim notes, and applying them as written failed its own preservation gate on 33 of 52 files — true text lost, or new falsehoods installed, and two of the new claims rotted during the run itself — while deleting at minimum span converged in one repair round and left the corpus smaller. Consequences: **never a dated note per claim** — a dated note only where the falsification is itself the lesson (D-T5); a moved pointer is fixed by citing the symbol or by cutting the pointer, never by a new line number; when unsure whether a fragment is still true, keep it — a kept true claim costs nothing, a deleted one is unrecoverable in an unversioned directory. **The sweep's own proposals never grow memory** — not file count, not index bytes, not total corpus bytes; a sweep that grows any of them has restated staleness, not removed it. A lesson this run's §A captured is budgeted as on any other run.

1. **Mechanical sweep first** (deterministic checks over model judgment): every memory entry naming a file, symbol, flag, PR, or branch — grep or `gh` it. Gone → flag it with the failed command as evidence. Moved → cite the symbol, or cut the pointer; never a new line number.
...
- `proposals[]` — each:
  - `kind`: `memory-new` | `memory-amend` | `memory-trim` | `memory-rewrite` | `memory-delete` | `index-line` | `doc-edit` — a CURATE staleness fix is a `memory-trim` unless the whole file is dead
  ...
  - `content`: the **exact text to write** — not for `memory-trim`, whose write is specified by `find` / `replace` below. The PM applies it verbatim; never hand back a description of what to write.
  - `find` / `replace`: for `memory-trim` — the exact span to remove and its replacement (empty, or the shorter phrase that stays true). Choose the span so it occurs EXACTLY ONCE in the file — widen it until it does; a minimum-span cut is short by construction and short spans collide — and so the surviving text still reads: absorb an orphaned connective, list comma, or emphasis marker rather than leave `A, B into` or a bolded generic word. A span inside a table row keeps every `|` and line break intact. Run the no-new-specifics check on your own `replace` before you emit it and state the result in the proposal — no commit sha, PR or issue number, `file:line` cite, date, or count absent from the file; a `replace` that fails it is a rewrite, not a trim, so re-cut it or drop the proposal. The PM applies what you hand it verbatim (`run-story` §10c) and does not re-check this. A trim removes text, so its `removes_content` is `true`.
...
- `memory_budget` — file count, index bytes, and total corpus bytes, before → after if every proposal is applied. Net growth needs a justifying sentence. On a CURATE run the §B sweep's own proposals may not grow any of the three (§B); a lesson §A captured on the same exit is budgeted as on any other run. It is a ceiling on growth, never a reason to shrink — item 3 governs that by claim count.
