...
Bands are planning estimates — calibrate against 10.2 actuals and report build spend and verify spend as **two separate numbers** after each run (§6.5).
...
5. **Token reporting — build and verification as two numbers.** After each phase: actual per-story spend vs the §5 band, one line each, with **build spend and verification spend reported separately**. A story that lands on-band for build and 3× over on verification is a verification-scoping problem, not a build-estimation problem; one blended number hides which, and the wrong lever gets pulled at recalibration. Bands recalibrate after Phase 10.2 actuals.
